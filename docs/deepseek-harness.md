...
Orbis 的 `dsh` 后端默认接入官方 Web 会话：Host 连接本机 `dsh web`，手机和浏览器通过同一 Session 共享实时事件。旧 ACP 适配仍保留用于兼容测试和故障回退。Windows 桌面与 Android 都提供 DeepSeek 入口；Pi、Codex 和 DSH 可以同时启用。
...
Harness home 默认是 `~/.dsh`，可用 `DSH_HOME` 覆盖。Orbis 默认连接 `web` profile；已有 Web 服务可在桌面设置中填写完整的 loopback 启动链接（含 `?token=`），否则 Host 会按 descriptor 复用或启动自己的 Web 服务。home 级 `$DSH_HOME/cordis.patch.yml` 会在 profile 后应用；同一行的 `config` 是整体替换，不是字段深合并。修改供应商后重新打开 Web 工作台。
...
桌面「打开 DeepSeek Harness」打开 Host 使用的同一个 `dsh web` 工作台。Orbis 会复用已有的 loopback Web 服务；没有时在 `127.0.0.1` 启动持久服务并保存带 token 的 descriptor。Host 断开不会关闭 Web 服务，因此浏览器、手机和 Host 能重新接回同一会话。Web 的模型选择、消息、实时流和队列操作会同步给所有客户端。
...
- Host 不拥有 Web 服务的浏览器生命周期，只持有认证连接；恢复已有会话前不要让另一个写入者同时修改同一 Session。
...
- Web transport 的请求超时只结束该 RPC；连接断开后会自动重连并恢复各会话的 follow/snapshot。Host 退出不会关闭 dsh Web 服务，浏览器和手机仍可继续使用；`/quit` 只归档指定会话。
...
`node scripts/test-dsh-web.mjs` 调用已安装的真实 dsh，但创建独立临时 `DSH_HOME` 和工作目录，模型请求仅发往本机回环测试服务，不消费真实 API 额度。它验证 Web URL/token、双客户端同会话 follow、实时 token/tool 流、队列撤回、session cancel 和工具 round-trip。测试不会修改用户已有 dsh 会话或凭据。
