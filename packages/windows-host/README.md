...
2. Review Pi/Codex/DeepSeek Harness on the Agent page. Check updates, then pause Host before installing or updating. Choose the detected npm installation to update that exact directory, or choose an Orbis independent installation to retain the previous version. Manage providers under each Agent: basic forms configure a compatible API, while advanced JSON preserves native options. Existing native configuration is imported on refresh.
...
DeepSeek Harness is opt-in under Settings. Enable it and restart the Host, then choose DeepSeek when creating a session on Android. The Agent page opens the same authenticated `dsh web` workspace that the phone observes; Orbis keeps the Web service alive independently of the Host transport. Configuration and supported capabilities are documented in [DeepSeek Harness integration](../../docs/deepseek-harness.md).

Provider switching follows CC Switch's native-file model: Codex saves the outgoing `auth.json`/`config.toml` snapshot and applies the selected snapshot; Pi adds or removes explicit nodes in `models.json.providers` while retaining `/login`, `auth.json`, and default-model settings; DSH writes the home-level `cordis.patch.yml`. Existing Web sessions keep their process and credentials; reopen the Web workspace after changing DSH provider settings. Orbis keeps its provider catalog in the local Host state directory. API keys and Codex auth snapshots stay on the computer; the Android app receives names, IDs and enabled state only. Codex and DSH refuse switching while a task or approval is in progress. Reopen existing sessions and standalone terminals after switching; reopen Pi to load changed explicit providers, then choose a model with `/model`. The Android Settings page can enable or switch providers already configured on the Host. Codex OAuth tokens may require signing in again after an older snapshot is restored; Orbis does not refresh them.
...
Agent downloads use CC Switch's npm `dist-tags` version check with a 15-second timeout and semantic version comparison. Pi/Codex track `latest`, without promoting stable users to alpha/beta channels. DSH recommends at least the published `0.1.7-rc.1` ACP version required by Orbis when its `latest` tag points to an older release; the page displays this compatibility requirement. A missing Agent is distinguished from an installation whose CLI cannot run. Multiple npm copies are listed in Installation history. Batch operations run Pi, Codex and DSH sequentially, continue after an individual failure, and report each failure.

Orbis pins npm to its Node interpreter and the selected installation prefix. Independent installations live under `%LOCALAPPDATA%\Orbis\agents`; npm verifies download integrity, then Orbis checks the package name, version, entry boundary and actual `--version` output before atomically saving the active-version record. Old independent versions can be revalidated and selected from Installation history. Cancelling terminates only the installer process tree and cleans its unfinished directory. Updating an existing npm installation changes that installation in place; cancellation or failure may require repairing it. Project dependencies and custom/other package-manager installations are not rewritten as global npm packages; choose an independent installation for those entries.
...
For development, run the built executable with `--runtime-root <repository>`. `--data-dir <directory> --host-state-dir <directory>` isolate desktop/Host data. `--smoke-test --screenshot <png>` renders the actual QML window, checks the bundled Host and exits; add `--smoke-agents` to capture the installer, history and batch dialogs without accepting them. Smoke tests automatically isolate Host identity if no state directory was supplied; they do not send emails or start agent sessions. `ORBIS_AGENT_INSTALL_ROOT` isolates downloaded Agent versions when testing the install lifecycle.
